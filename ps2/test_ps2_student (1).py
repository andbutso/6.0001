...
class TestPS2(unittest.TestCase):
    ...
    def test_play_game_warnings(self):
        ...
        with unittest.mock.patch('builtins.input',  make_input):
            ...
            if len(lines) > 8:
                ...
                self.assertTrue("1 guesses" in lines[10])
    ...
    def test_play_game_wildcard(self):
        global input_string
        random.seed(2032445)
        input_string = (letter for letter in ["r", "*", "-", "-", "*", "e","i", "k","j", "a"])
        with unittest.mock.patch('builtins.input',  make_input):
            student.hangman_with_help("racecar")
            global outputstr
            ...
            if len(lines) > 7:
                self.assertTrue("8 guesses" in lines[1])
                self.assertTrue("r#####r" in lines[1])
                self.assertTrue("8 guesses" in lines[2])
                self.assertTrue("revealed" in lines[2])
                self.assertTrue("r#c#c#r" in lines[2])
                self.assertTrue("6 guesses" in lines[3])
                self.assertTrue("r#c#c#r" in lines[3])
                self.assertTrue("6 guesses" in lines[4])
                self.assertTrue("abdeghijklmnopqstuvwxyz" in lines[4])
                self.assertTrue("6 guesses" in lines[5])
                self.assertTrue("abdeghijklmnopqstuvwxyz" in lines[5])
                self.assertTrue("4 guesses" in lines[6])
                self.assertTrue("rac#car" in lines[5])
                self.assertTrue("racecar" in lines[6])
                self.assertTrue("score" in lines[7])
                self.assertTrue("28" in lines[7])
            else:
                self.assertTrue(False)
...
failure_messages = {
    ...
    'test_play_game_wildcard': 'You do not handle wildcards correctly'
...
point_values = {
    'test_is_word_guessed' : .60,
    'test_is_word_guessed_repeated_letters' : .60,
    'test_get_guessed_word': .60,
    'test_get_guessed_word_empty_string': .60,
    'test_get_available_letters': .60,
    'test_get_available_letters_empty_string': .60,
    'test_play_game_short': .35,
    'test_play_game_short_fail': .35,
    'test_play_game_warnings': .35,
