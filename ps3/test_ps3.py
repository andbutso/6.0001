...
STOP_ITER_MSG = """For the given list of use commands executed in order %s,
when your hand is %s your code did not complete the game. You might want to check that you no longer prompt the user if they want to replay a hand/substitute a letter after they do it once.
        """
STOP_ITER_MSG_GAME = """For the given list of use commands executed in order %s,
...
when your hands are %s your code return score %s instead of the correct score %s.  
"""
...
class TestPlayHandAndGame(unittest.TestCase):
    ...
    def test_get_word_score(self):
        ...
        words = {("", 7):0, ("it", 7):2, ("was", 7):66, ("weed", 6):224,
                 ("scored", 7):450, ("WaYbILl", 7):945, ("Outgnaw", 7):693,
                 ("fork", 7):264, ("FORK", 4):396}
        try:
...
# end of test_is_valid_word
    ...
    def test_deal_hand_wildcard(self):
        ...
        self.assertEqual(vowels_sum, num_vowels,"The wildcard in deal_hand should replace a constonant. The number of vowels %s =int(math.ceil(%s / 3)) should remain unchanged. Your dealt hand is : %s" %(num_vowels,9,str(hand)))
    def test_wilcard_score(self):
        # dictionary of words and scores WITH wildcards
        words = {("!oney", 7):259, ("!ows", 6):168, ("!ails", 7):148}
        for (word, n) in words.keys():
            try:
                score = ps3.get_word_score(word, n)
        ...
        try:
            ...
            with mock.patch('builtins.input', side_effect=inputs):
                ...
                if result == None:
                    sys.stdout = saved_stdout
                    self.fail(NOT_IMPLEMENTED)
            self.assertEqual(result, 170, WRONG_SCORE_GAME %(inputs, h1, result, 170))
            ps3.deal_hand = student_deal_hand
        except StopIteration as e:
            ps3.deal_hand = student_deal_hand
            sys.stdout = saved_stdout
        ...
        try: #mock input for game
            with mock.patch('builtins.input', side_effect=inputs):
                ...
                if result == None:
                    sys.stdout = saved_stdout
                    self.fail(NOT_IMPLEMENTED)
            self.assertEqual(result, 376, WRONG_SCORE_GAME_REPLACE %(inputs, [h1,h2], 'd', result, 376))   
