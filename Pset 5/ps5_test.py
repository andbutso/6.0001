...
class ProblemSet5(unittest.TestCase):
    ...
    def test0PhraseTrigger(self):
        ...
        separate  = 'The purple blob over there is a cow.'
        brown     = 'How now brown cow.'
        badorder  = 'Cow!!! Purple!!!'
        nospaces  = 'purplecowpurplecowpurplecow'
        nothing   = 'I like poison dart frogs.'

        s1 = PhraseTrigger('PURPLE COW')
        s2 = PhraseTrigger('purple cow')
        for trig in [s1,s2]:
            self.assertTrue(trig.is_phrase_in(cuddly), "is_phrase_in returned False when the phrase was contained in text")
            self.assertTrue(trig.is_phrase_in(exclaim), "is_phrase_in returned False when the words were separated by exclamation marks.")
            self.assertTrue(trig.is_phrase_in(symbols), "is_phrase_in returned False when the words were separated by assorted punctuation.")
            self.assertTrue(trig.is_phrase_in(spaces), "is_phrase_in returned False fire when the words were separated by multiple spaces.")
            self.assertTrue(trig.is_phrase_in(caps), "is_phrase_in returned False when the phrase appeared with both uppercase and lowercase letters.")
            self.assertTrue(trig.is_phrase_in(exact), "is_phrase_in returned False when the words in the phrase were the only words in the title.")
            
            self.assertFalse(trig.is_phrase_in(plural), "is_phrase_in returned True when the words in the phrase were contained within other words.")
            self.assertFalse(trig.is_phrase_in(separate), "is_phrase_in returned True when the words in the phrase were separated by other words.")
            self.assertFalse(trig.is_phrase_in(brown), "is_phrase_in returned True when only part of the phrase was found.")
            self.assertFalse(trig.is_phrase_in(badorder), "is_phrase_in returned True when the words in the phrase appeared out of order.")
            self.assertFalse(trig.is_phrase_in(nospaces), "is_phrase_in returned True when words were not separated by spaces or punctuation.")
            self.assertFalse(trig.is_phrase_in(nothing), "is_phrase_in returned True when none of the words in the phrase appeared.")
    ...
    def test3BeforeAndAfterTrigger(self):
        ...
        now = datetime(2017, 3, 4, 23, 59, 59)
        ...
        s1 = BeforeTrigger('4 Mar 2017 23:59:59')
        s2 = AfterTrigger('4 Mar 2017 23:59:59')
